...
/**
 * Login-gated services whose homepages do not expose public deep links
 * to specific events. These root URLs are acceptable per CLAUDE.md and
 * should not be flagged as quality issues.
 */
export const ACCEPTABLE_ROOT_DOMAINS = new Set([
  'peacocktv.com',
  'max.com',
  'discoveryplus.com',
  'discoveryplus.co.uk',
...
/**
 * Like isRootUrl but returns true only for root URLs that are NOT in the
 * acceptable-root allowlist. Use this when flagging quality issues.
 */
export function isProblematicRootUrl(url) {
  if (!isRootUrl(url)) return false;
  try {
    const host = new URL(url).hostname.replace(/^www\./, '').toLowerCase();
    return !ACCEPTABLE_ROOT_DOMAINS.has(host);
  } catch {
    return true;
  }
}
  ...
  for (const [geo, geoData] of Object.entries(broadcast.geos)) {
    ...
    if (geoData.primary?.url) {
      ...
      if (validation.valid && validation.isDeepLink) {
        ...
        // Acceptable root URLs (login-gated services per CLAUDE.md) count
        // as valid; only "problematic" roots flag a quality issue.
        if (isProblematicRootUrl(geoData.primary.url)) {
          results.rootUrls++;
        ...
        // Valid URL, not a root, not a recognised deep link — typically a
        // legitimate topic/category page (e.g., sporza.be/wielrennen/veldrijden/,
        // youtube.com/@ChannelName). These are real broadcaster pages where
        // deep linking to a specific event isn't available. Count as valid.
        results.validUrls++;
      } else {
        results.invalidUrls++;
      ...
      for (const alt of geoData.alternatives) {
        if (alt.url) {
          ...
          if (validation.valid && validation.isDeepLink) {
            ...
            if (isProblematicRootUrl(alt.url)) {
              results.rootUrls++;
            ...
            // Valid URL, non-root, non-deep-link — legitimate topic/category
            // page or channel URL. Count as valid.
            results.validUrls++;
          } else {
            results.invalidUrls++;
