...
import { recordApiCall } from './cost-ledger.js';
...
async function perplexityRequest(endpoint, data) {
  ...
  recordApiCall('perplexity', endpoint, { status: response.status });
...
export async function searchRaceBroadcast(raceName, year = 2026, options = {}) {
  const query = `Find broadcasters and streaming URLs for the ${year} edition of ${raceName} specifically.

CRITICAL: every URL you return must point to the ${year} edition. Reject any URL that resolves to a prior edition (e.g., ${year - 1} or older). If a broadcaster has no published ${year} event page yet, return their hub or sport-section page rather than an old-year event page.

Provide:
- Official broadcasters by region (US, UK, Europe, AU, CA, etc.) — each with the deepest URL that is confirmed to be the ${year} edition
- Streaming platforms carrying the race in ${year}
- Race start time (local and UTC) for the ${year} edition
...
- HBO Max / TNT / Discovery+ coverage details if applicable

Format by region/platform. Do not return URLs whose pages reference ${year - 1} as the current edition.`;
...
// ============================================
// BROADCASTER RESEARCH FUNCTIONS
// ============================================

/**
 * Geographic region name mapping
 */
const GEO_NAMES = {
  'US': 'United States',
  'CA': 'Canada',
...
/**
 * Search for broadcaster licensing information by geographic region
 * Returns which platforms/networks have rights to broadcast cycling
 *
 * @param {string} geo - Geographic region code (US, CA, UK, EU, AU)
 * @param {number} year - Year to search for (default 2026)
 * @param {object} options - Perplexity search options
 */
export async function searchBroadcastersByGeo(geo, year = 2026, options = {}) {
  const geoName = GEO_NAMES[geo.toUpperCase()] || geo;

  const query = `Professional cycling TV broadcast and streaming rights ${geoName} ${year}:

For ${geoName} market, provide:

1. GRAND TOURS (Tour de France, Giro d'Italia, Vuelta a España):
   - Primary broadcaster/streaming platform
   - Live coverage vs highlights only
   - Subscription required?

2. SPRING CLASSICS (Paris-Roubaix, Tour of Flanders, Milan-San Remo):
   - Which platforms show these races?
   - Live or delayed coverage?

3. UCI WORLD CHAMPIONSHIPS:
   - Who has the broadcast rights?
   - YouTube availability?

4. WORLD TOUR RACES:
   - General World Tour coverage availability
   - Key platforms for World Tour access

5. YOUTUBE CHANNELS:
   - Official broadcaster YouTube channels
   - What content do they post?

Format as structured list by race category.`;

  console.log(`📺 Researching broadcasters for: ${geoName} ${year}`);
...
/**
 * Search for race broadcast availability across multiple geographic regions
 *
 * @param {string} raceName - Name of the race
 * @param {number} year - Race year (default 2026)
 * @param {string[]} geos - Array of geo codes to search (default: ['US', 'CA', 'UK'])
 * @param {object} options - Perplexity search options
 */
export async function searchRaceBroadcastMultiGeo(raceName, year = 2026, geos = ['US', 'CA', 'UK'], options = {}) {
  const geoNames = geos.map(g => GEO_NAMES[g.toUpperCase()] || g);
  const geoList = geoNames.join(', ');

  const query = `${raceName} ${year} TV and streaming broadcast coverage:

For each region (${geoList}), provide:

| Region | Platform | Live/Replay | Subscription | Official URL |
|--------|----------|-------------|--------------|--------------|

Include:
- Primary broadcaster for each region
- Whether live coverage is available
- If subscription is required
- Alternative viewing options (YouTube highlights, etc.)
- Any geo-restrictions or blackouts
- Official YouTube channel posting highlights

Format as structured table with one row per region/platform combination.`;

  console.log(`📺 Multi-geo broadcast search: ${raceName} ${year} (${geoList})`);
...
/**
 * Verify if a specific broadcaster carries a specific race
 *
 * @param {string} broadcasterName - Name of broadcaster (e.g., "Peacock", "FloBikes", "Eurosport")
 * @param {string} raceName - Name of the race
 * @param {number} year - Race year (default 2026)
 * @param {object} options - Perplexity search options
 */
export async function verifyBroadcasterRace(broadcasterName, raceName, year = 2026, options = {}) {
  const query = `Does ${broadcasterName} broadcast ${raceName} ${year}?

Confirm:
1. Does ${broadcasterName} have broadcast rights for ${raceName}?
2. Is it live coverage or replay/on-demand only?
3. Full race coverage or highlights only?
4. Is there a blackout or geo-restriction?
5. What subscription tier is required?
6. Does ${broadcasterName} post YouTube highlights for this race?

Provide definitive answer with source.`;

  console.log(`📺 Verifying: ${broadcasterName} → ${raceName} ${year}`);
