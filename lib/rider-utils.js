...
export function loadOutsiders() {
  const outsidersPath = join(__dirname, '..', 'data', 'outsiders.json');
  if (!existsSync(outsidersPath)) return { lastUpdated: null, riders: [] };
  return JSON.parse(readFileSync(outsidersPath, 'utf-8'));
}

export function saveOutsiders(outsidersData) {
  const outsidersPath = join(__dirname, '..', 'data', 'outsiders.json');
  outsidersData.lastUpdated = new Date().toISOString();
  writeFileSync(outsidersPath, JSON.stringify(outsidersData, null, 2));
  console.log(`✅ Saved ${outsidersData.riders.length} outsiders to outsiders.json`);
}
 ...
 */
export async function scrapeRankings(limit = 50) {
 ...
 */
export async function scrapeAllRiders(limit = 50, delayMs = 2000) {
...
const RACE_SLUG_MAPPING = {
  ...
  'omloop-het-nieuwsblad': 'omloop-van-het-hageland', // Close match
  'volta-a-catalunya': 'volta-ciclista-catalunya',
 ...
 */
export function saveRaceData(raceData) {
  const raceDataPath = join(__dirname, '..', 'data', 'race-data.json');
 ...
 * Tries direct match first, then uses mapping
 */
 ...
 * Adds topRiders array to each race in race-data.json
 */
export function populateRaceRiders(ridersData = null, raceData = null, outsidersData = null) {
  console.log('\n🚴 Populating races with top riders...');

  if (!ridersData) ridersData = loadRiders();
  if (!raceData) raceData = loadRaceData();
  if (!outsidersData) outsidersData = loadOutsiders();

  const raceIds = new Set(raceData.races.map(r => r.id));
  const raceRidersMap = new Map();

  // Combined source: ranked first, then outsiders (carry isOutsider flag)
  const allSources = [
  ...
  for (const rider of allSources) {
    ...
    for (const programRace of rider.raceProgram.races) {
      ...
      if (!raceId) continue;
      if (!raceRidersMap.has(raceId)) raceRidersMap.set(raceId, []);
      const entry = {
        id: rider.id,
        name: rider.name,
        team: rider.team,
  ...
  // Past-race gate: rider programs are forward-looking, so for races that
  // have already happened we leave existing topRiders untouched (curated
  // historical startlists must survive a refresh).
  ...
  console.log(`   ✅ ${racesWithRiders} races have top riders`);
...
/**
 * Run the full rider-to-race population and save
 */
export async function updateRaceRiders() {
  const ridersData = loadRiders();
