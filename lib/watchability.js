...
/** Flames (1–5) for a one-day race, or the rounded AVERAGE of stage flames for a
 *  stage race (so the calendar card reflects the whole race, not one banger stage).
 *  Returns null if no results, 0 if cancelled. */
export function flamesForRace(raceId, { resultsDir = 'data/results', rating = 0 } = {}) {
  ...
  if (fs.existsSync(stagesDir)) {
    for (const f of fs.readdirSync(stagesDir)) {
      if (f.startsWith(`${raceId}-stage-`) && f.endsWith('.json')) {
        try { stageFlames.push(scoreResult(JSON.parse(fs.readFileSync(path.join(stagesDir, f), 'utf8')), { rating }).flames); } catch {}
      }
    }
  }
  if (stageFlames.length) return Math.round(stageFlames.reduce((a, b) => a + b, 0) / stageFlames.length);
