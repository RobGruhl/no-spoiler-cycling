...
const RX = {
  upset: /\b(career[- ]best|biggest win of (?:his|her) career|standout ride of (?:his|her) career|maiden|first (?:ever )?(?:pro|professional|world ?tour|uci|grand tour)?\s*(?:road |stage )?win|first win|first (?:cobbled )?(?:monument|classic|grand tour|major|world ?tour)|first (?:cobbled )?(?:monument|classic)|long[- ]awaited|surprise|surprised|upset|outsider|unexpected|stunn|shock|breakthrough|unheralded|neo[- ]?pro|stole|audacious|against the odds|denied the home|fairy[- ]?tale|sensational)\b/i,
  longRangeMove: /\b(\d{2,})\s?-?\s?(?:km|kilometre|kilometer)\b[^.]{0,40}?(?:to go|to race|out|remaining|from (?:the )?(?:finish|line)|left)|(?:forty|fifty|sixty|seventy|eighty|ninety|hundred)[- ]?(?:odd|some)?[- ]?(?:km|kilometre|kilometer)|from (?:way |far )?(?:out|distance)|long[- ]range|went clear early/i,
  ...
  twoUp: /\b(two[- ]up|2[- ]up|head[- ]to[- ]head|duel|the duo|duo (?:formed|stayed|went|rode|never)|the two of them|two[- ]rider (?:move|group)|reduced group sprint|small[- ]group sprint|from a (?:reduced|select|small) group|select group)\b/i,
  ...
  hardparcours: /\bcobbl\w*|\bpav(?:é|e)\w*|\bgravel\w*|\bribin\w*|\bsterrato|\bmur\b|\bwall\b|\bberg\b|summit finish|hors cat|hc climb|attritional|savage|brutal|leg[- ]breaking/i,
  ...
  // a universally-celebrated / emotional win (everyone loved that they got it)
  beloved: /\b(dedicat(?:e|ed|es|ion)|in memory|in tribute|tribute to|more than a (?:win|victory)|emotional|in tears|wept|fan[- ]favou?rite|beloved|popular (?:win|victory|winner|champion)|long[- ]awaited|fairy[- ]?tale|dream (?:win|come true|realised|realized)|redemption|home (?:hero|favou?rite|crowd|win)|sentimental|universally (?:loved|popular|admired|celebrated)|finally (?:got|won|claimed|broke)|after years of)\b/i,
};
...
export function detectArchetype(text, gapSec, stageType) {
  ...
  // two-up / duel BEFORE solo: a duel is the story even if the winner "rode away" at the end
...
export function scoreResult(data, opts = {}) {
  ...
  // long-range aggression — a winning move that went from far out (the "from 100km
  // is fun" principle). longSolo already banks distance in its base, so skip it.
  if (arch.key !== 'longSolo' && RX.longRangeMove.test(text)) { score += 8; mods.push(['long-range', 8]); }

  // beloved / emotional win — scanned over a wider sentiment text (incl. aftermath +
  // quotes) so dedications, tributes and fairy-tales count. Blended only (spoiler-safe).
  const sentimentText = text + ' ' + JSON.stringify(data.aftermath || '');
  if (RX.beloved.test(sentimentText)) { score += 8; mods.push(['beloved', 8]); }
  ...
  if (!cancelled) {
    ...
    if ((arch.key === 'longSolo') || RX.longRangeMove.test(text) || (lastKm != null && lastKm >= 30)) tags.push('Long-range racing');
...
// ---- build-time helpers ----
// Flame ratings are PRECOMPUTED by parallel Opus subagents judging each watchable
// unit against the descriptive rubric (see docs/WATCHABILITY.md + scripts/score-
// watchability-merge.js), and stored in data/results/watchability.json:
//   { races: { "<one-day-race-id>": <1-5> }, stages: { "<race-id>-stage-N>": <1-5> }, notes: {...} }
// The heuristic scoreResult() below is retained only as a reference/fallback tool;
// the site build reads the LLM judgements via these loaders.

let _watchCache = null;
function loadWatch(resultsDir = 'data/results') {
  if (_watchCache) return _watchCache;
  try { _watchCache = JSON.parse(fs.readFileSync(path.join(resultsDir, 'watchability.json'), 'utf8')); }
  catch { _watchCache = { races: {}, stages: {} }; }
  return _watchCache;
}
const _flames = v => (v && typeof v === 'object') ? (v.flames ?? null) : (typeof v === 'number' ? v : null);

/** Flames (1–5) for a ONE-DAY race (the only race-level watchable unit), else null. */
export function flamesForRace(raceId, { resultsDir = 'data/results' } = {}) {
  return _flames(loadWatch(resultsDir).races?.[raceId]);
}

/** Flames (1–5) for a single stage, else null. */
export function flamesForStage(raceId, n, { resultsDir = 'data/results' } = {}) {
  return _flames(loadWatch(resultsDir).stages?.[`${raceId}-stage-${n}`]);
}

/** Aggregate flames (1–5) for a whole tour / stage race, else null. */
export function flamesForTour(raceId, { resultsDir = 'data/results' } = {}) {
  return _flames(loadWatch(resultsDir).tours?.[raceId]);
}
