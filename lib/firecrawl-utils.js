...
// Check lazily (at call time), NOT at import time. Throwing at module load made
// `import('./lib/firecrawl-utils.js')` crash the whole process when the key is
// absent (e.g. a fresh cloud checkout with no .env) — before any caller could
// detect-and-report. Mirrors perplexity-utils.js's checkApiKey().
...
async function firecrawlRequest(endpoint, data) {
  ...
  recordApiCall('firecrawl', endpoint, { status: response.status });
...
export async function discoverBroadcastUrls(broadcasters, raceName, year = 2026, geos = ['US', 'CA', 'UK']) {
  ...
  // Drop URLs whose path contains a year token that isn't the expected year.
  // Catches FloBikes /events/<id>-2025-... and SBS /...-2025-mens-race-... cheaply.
  const YEAR_IN_URL = /\b(20[0-2]\d)\b/g;
  const expectedYearStr = String(year);
  const filterByYear = (urlList) => urlList.filter(r => {
    if (!r?.url) return false;
    const yrs = [...r.url.matchAll(YEAR_IN_URL)].map(m => m[1]);
    if (yrs.length === 0) return true; // year-agnostic URL — keep
    return yrs.includes(expectedYearStr); // keep only if expected year is present
  });
  ...
  for (const geo of geos) {
    ...
    if (geoData.primary) {
      for (const broadcaster of geoData.primary) {
        ...
        const rawResults = await searchBroadcasterSite(domain, raceName, year, { limit: 3 });
        const searchResults = filterByYear(rawResults);
    ...
    if (geoData.secondary) {
      for (const broadcaster of geoData.secondary) {
        ...
        const rawResults = await searchBroadcasterSite(domain, raceName, year, { limit: 2 });
        const searchResults = filterByYear(rawResults);
