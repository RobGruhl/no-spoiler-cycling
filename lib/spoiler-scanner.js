...
// Result verbs a spoiler video title uses. Matched on the VERB itself (not a
// "<subject> verb" shape) so accented rider names — Pogačar, Roglič — that break
// ASCII \w word-boundaries can't slip a leak through. Highlights titles are
// descriptive ("Stage 5 Highlights") and don't contain these.
const TITLE_RESULT_VERB_RE = /\b(wins?|takes?\s+(?:the\s+)?(?:win|victory|stage|gc|lead|overall|title|jersey|maillot|maglia)|triumphs?|solos?\s+(?:to|clear|home)|seals?|clinches?|conquers?|defends?|outsprints?|outkicks?|storms?\s+to|powers?\s+to|holds?\s+off|wraps?\s+up)\b/i;

/**
 * Strict result-leak check for a short VIDEO/LINK TITLE we're about to attach to
 * the spoiler-FREE calendar (e.g. a YouTube highlights title). A safe highlights
 * title is purely descriptive: "Stage 5 Highlights", "Locarno", "Extended
 * Highlights - Stage 3". This is deliberately STRICTER than scanRaceForSpoilers
 * and has NO false-positive guards — for a title we prefer to over-reject (skip the
 * video) than to leak a result.
 *
 * LIMIT: it only catches keyword/verb leaks. A name-only spoiler with no result
 * word ("Pogačar's masterclass") has no keyword and WILL pass here — the footage
 * routine's trusted-channel allowlist + LLM judgment are the primary defense; this
 * is the mechanical backstop for the obvious cases. See
 * docs/daily-calendar-footage-routine.md.
 *
 * @param {string} title
 * @returns {boolean} true if the title looks like it leaks a result
 */
export function titleLeaksResult(title) {
  if (!title || typeof title !== 'string') return false;
  const kw = new RegExp(KEYWORD_RE.source, 'i'); // stateless copy (KEYWORD_RE is /g)
  if (kw.test(title)) return true;
  for (const re of PHRASE_HITS) { if (new RegExp(re.source, 'i').test(title)) return true; }
  return TITLE_RESULT_VERB_RE.test(title);
}
