...
- **Auto-found videos are QUARANTINED until a human approves them.** Every footage
  entry `scripts/set-race-footage.js` writes carries
  `review: {status:"pending", addedBy:"auto", added}`, and the generators render
  **only** `approved` entries (`lib/footage-review.js`, fail-closed). So a video
  this routine commits does **not** appear on the site until it clears the batched
  human review (see **Weekly footage review** below). This render gate is the
  enforcing backstop; the vetting below is the first filter, not the last.
- **Vetting still matters — when in doubt, skip.** A missing highlight is fine; a
  leaked result on the spoiler-free calendar is the worst possible failure of this
  project. The quarantine gate means a bad pick waits for review instead of going
  live, but don't lean on it — reject obviously-unsafe candidates at discovery.
...
5. **Regression gate is a hard stop.** Before committing, `npm test` must report
   **0 fail**. It catches calendar breakage, any spoiler text that reached a scanned
   field (e.g. a watchNote), and — via `scripts/test-footage-review.js`, now part of
   `npm test` — proves that **no pending/rejected footage reached the build**. It
   still cannot judge whether a video's *content* is a spoiler; the human review gate
   (guardrail 3 + Weekly footage review) is your defense there.
...
Every entry the tool writes lands as `review.status = "pending"` (quarantined) — it
is committed to `data/race-data.json` but renders **nowhere** until a human approves
it in the weekly review. This is automatic; the routine passes no review flag. (Do
**not** pass `--approved` — that flag is for a trusted human doing a manual backfill,
never for this unattended routine.)
...
Committed as review-pending (quarantined) — renders only after human approval.
<note any skipped items + generic reason>"
[ "$N" -gt 0 ] && git push origin main
```
Because every new entry is quarantined (`review.status = pending`), the rendered
`race-details/*.html` pages usually do **not** change — the commit is typically just
`data/race-data.json`. That's expected: you're queuing videos for review, not
publishing them. Stage the race-detail pages anyway (harmless if unchanged).
...
Frame what you did as **"queued N for review,"** not "published" — nothing goes live
until the weekly review approves it. A handy tail: "N video(s) queued for review —
run `node scripts/review-footage.js --list`."

## Weekly footage review (human gate — do this batched, not daily)

Auto-found videos sit `pending` and render nowhere until approved. Clear the queue
in one batched pass (weekly, or whenever the queue is worth a look — the daily
routine tells you the count):

```bash
node scripts/review-footage.js --list        # each pending entry: id, race, stage,
                                              # LIVE oEmbed title + channel, duration, URL
```

For each entry, apply the same judgment as guardrail 3 (neutral title, trusted
channel, no name-only spoiler, right edition). Titles are fetched live and shown, not
stored — `data/race-data.json` ships in the public repo, so no unreviewed title is
persisted there. Then, in one batch:

```bash
node scripts/review-footage.js --approve <id> [<id>…]   # publishes on next build
node scripts/review-footage.js --reject  <id> [<id>…]   # quarantines permanently
npm run build:all && npm test                           # render approvals; gate stays green
git add data/race-data.json race-details && git commit -m "footage: weekly review" && git push origin main
```

`--reject` keeps the record (URL retained, status `rejected`) so the daily routine's
work-list sees the slot as filled and never re-adds the same video. Approved entries
render on the next `build:all`; rejected/pending never do.
