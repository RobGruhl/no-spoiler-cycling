...
function renderWatchSection(race) {
  ...
  const highlightsHtml = renderYoutubeHighlightsBlock(race);
...
function renderYoutubeHighlightsBlock(race) {
  ...
  const items = Array.isArray(yh)
    ...
    : [{ url: yh, notes: race.videoNote }];
  if (!items.length) return '';

  const cards = items.map(item => {
    const stageLbl = item.stage ? `Stage ${item.stage} · ` : '';
    const typeLbl = item.type ? htmlEscape(item.type.replace(/-/g, ' ')) : 'highlights';
    const title = item.title || 'YouTube Highlights';
    const note = item.notes || '';
    return `<a class="yh-card" href="${htmlEscape(item.url)}" target="_blank" rel="noopener">
      <div class="yh-card-head"><span class="yh-icon">▶</span><span class="yh-title">${htmlEscape(title)}</span></div>
      <div class="yh-card-meta mono">${stageLbl}${typeLbl}${item.channel ? ' · ' + htmlEscape(item.channel) : ''}</div>
      ${note ? `<div class="yh-card-note">${htmlEscape(note)}</div>` : ''}
    </a>`;
  }).join('');

  return `<div class="yh-block">
    <div class="yh-lbl mono">Spoiler-safe highlights</div>
    <div class="yh-cards">${cards}</div>
  </div>`;
}
...
.yh-block{margin-top:18px;border:1px solid var(--rule);padding:14px}
.yh-lbl{font-size:11px;font-weight:600;letter-spacing:.2em;text-transform:uppercase;color:var(--ink-3);margin-bottom:10px}
.yh-cards{display:grid;grid-template-columns:1fr;gap:10px}
@media (min-width:760px){.yh-cards{grid-template-columns:repeat(2,1fr)}}
.yh-card{display:block;padding:12px 14px;border:1px solid var(--rule);background:var(--paper);text-decoration:none;color:var(--ink);transition:border-color .12s,background .12s}
.yh-card:hover{border-color:var(--ink);background:var(--paper-2)}
.yh-card-head{display:flex;align-items:baseline;gap:8px;margin-bottom:4px}
.yh-icon{color:var(--signal);font-size:13px}
.yh-title{font-weight:500;font-size:14px;line-height:1.3}
.yh-card-meta{font-size:11px;letter-spacing:.06em;color:var(--ink-3);text-transform:lowercase;margin-bottom:6px}
.yh-card-note{font-size:12px;line-height:1.45;color:var(--ink-2)}
